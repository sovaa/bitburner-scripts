...
export async function main(ns) {
	...
	// remove all bought servers that are scanable from home, they
	...
	// recursively scan every server that is scanable from the
	// 'home' server; all servers in the game will be listed in
	// 'all_servers'
	...
	// sort each server in the game not by their distance, but
	// by their hack level, so we can print what next to hack
	const sorted = server_and_hack_lvl.sort(
		(s1, s2) => s1.level - s2.level,
	)

	// now print each server that we have NOT yet root access to,
	// but we DO HAVE a hack level high enough for to nuke
	for (let i = 0; i < sorted.length; i++) {
		...
		if (!ns.hasRootAccess(name)) {
			ns.tprint(
				...
				ns.getServerMaxMoney(name).toFixed(0).toString().padEnd(15)
			)
		}
	}
}
...
export function recursively_scan_these_servers(ns, s, all_servers) {
	...
	for (let i = 0; i < new_servers.length; i++) {
		// avoid an infinite loop, only add servers to scan that we
		// haven't scanned before
		if (all_servers.includes(new_servers[i])) {
			continue
		}

		// add the unique servers to the 'all_servers' list, which
		// we will be accessing in the main() function afterwards
